...
def load_data_by_sector(kind: str, main_sectors_only=False) -> pd.DataFrame:
    ...
    # Load the data
    path = DATA_DIR / "revenue-reports" / f"{kind}-by-sector.csv"
...
def load_monthly_collections() -> pd.DataFrame:
    """
    ...
    """
    # Load
    path = DATA_DIR / "revenue-reports" / "monthly-collections-tax.csv"
    out = pd.read_csv(path)
    ...
    return pd.concat(
        [
            ...
            .groupby(
                ["month", "year", "fiscal_year", "month_name", "fiscal_month"]
            )["total"]
            .sum()
            .reset_index()
